...
if ($mode == 'update') {
    ...
    $storefronts_data = [];
    $banner_data = Tygh::$app['view']->getTemplateVars('banner');

    foreach (reset($storefronts) as $storefront) {
        if ($banner_data['company_id'] == $storefront->storefront_id) {
            continue;
        }
        $storefronts_data[$storefront->storefront_id] = $storefront->name;
    }

    Tygh::$app['view']->assign('storefronts_data', $storefronts_data);
    if (!empty($_REQUEST['banner_data']['copy']) && !empty($_REQUEST['banner_id'])) {
        $banner_id = (int) $_REQUEST['banner_id'];
        $link_id = (int) ($_REQUEST['link_id'] ?? 0);
        $banner_type = $_REQUEST['banner_data']['type'];
        $storefront_ids = $_REQUEST['banner_data']['storefronts_banners'];
        ...
        foreach ($storefront_ids as $storefront_id) {
            if (!empty($child_banners)) {
                ...
                $_REQUEST['banner_data']['company_id'] = $storefront_id;
